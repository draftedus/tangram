...
fn main() -> Result<()> {
	...
	let model = tangram_core::tree::MulticlassClassifier::train(
		features_train,
		labels_train.clone(),
		train_options,
		&mut |_| {},
	...
	// compute metrics
	let mut metrics = tangram_core::metrics::ClassificationMetrics::new(100);
	metrics.update(tangram_core::metrics::ClassificationMetricsInput {
