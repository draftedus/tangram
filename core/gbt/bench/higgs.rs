...
fn main() -> Result<()> {
	...
	let model =
		tangram::gbt::BinaryClassifier::train(features_train, labels_train.clone(), train_options);
	let end = Instant::now();
	println!("duration: {:?}", end - start);

	// compute accuracy
	let n_features = features.ncols();
	let columns = features.view().columns;
	let mut features_ndarray = unsafe { Array2::uninitialized((features.nrows(), n_features)) };
