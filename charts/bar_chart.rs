...
fn draw_bar_chart(
	...
	let y_min = options.y_min.unwrap_or_else(|| {
		series
			.iter()
			.flat_map(|series| series.data.iter().map(|p| p.y.unwrap_or(f64::INFINITY)))
	...
	let mut y_max = options.y_max.unwrap_or_else(|| {
		series
			.iter()
			.flat_map(|series| series.data.iter().map(|p| p.y.unwrap_or(f64::NEG_INFINITY)))
	...
	let bar_group_width = (chart_box.w - group_gap * (n_categories + 1.0)) / n_categories;
	...
	for (series_index, series) in series.iter().enumerate() {
		for (point_index, point) in series.data.iter().enumerate() {
			...
			let rect = Rect {
				h: (y / (y_max - y_min)) * chart_box.h,
				w: (bar_group_width - chart_config.bar_gap * (series_len - 1).to_f64().unwrap())
					/ series_len.to_f64().unwrap(),
				x: chart_box.x
					+ (group_gap + (group_gap + bar_group_width) * point_index.to_f64().unwrap())
					+ (chart_config.bar_gap + bar_width) * series_index.to_f64().unwrap(),
				y: chart_box.y + ((y_max - y) / (y_max - y_min)) * chart_box.h,
			};
	...
	let overlay_info = BarChartOverlayInfo { chart_box };
...
fn draw_bar_chart_overlay(
	options: DrawOverlayOptions<BarChartOverlayInfo, BarChartHoverRegionInfo>,
) {
