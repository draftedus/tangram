use crate::{
	...
	production_stats::{
		ProductionColumnStats, ProductionColumnStatsOutput, ProductionStats, ProductionStatsOutput,
	},
};
use anyhow::Result;
...
pub struct GetProductionColumnStatsOutput {
	pub date_window: DateWindow,
	pub date_window_interval: DateWindowInterval,
	pub overall: ProductionColumnStatsOutput,
	pub intervals: Vec<ProductionColumnStatsOutput>,
}
...
pub async fn get_production_column_stats(
	...
	date_window_interval: DateWindowInterval,
	timezone: Tz,
) -> Result<GetProductionColumnStatsOutput> {
	...
	// initialize the intervals with start and end dates
	let mut intervals: Vec<ProductionColumnStats> = (0..n_intervals.to_u32().unwrap())
		.map(|i| {
			// determine the start and end dates for the interval
			let start = match date_window_interval {
				DateWindowInterval::Hourly => start_date.with_hour(i).unwrap(),
			...
			ProductionColumnStats::new(&column_stats)
		})
		.collect();
	// merge each hourly production stats
	// entry into its corresponding interval
	...
	let overall = intervals
		...
		.finalize();
	// finalize the intervals
	let intervals: Vec<ProductionColumnStatsOutput> = intervals
		.into_iter()
		.map(|stats| stats.finalize())
		.collect();
	Ok(GetProductionColumnStatsOutput {
		date_window,
		date_window_interval,
		overall,
		intervals,
