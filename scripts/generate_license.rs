...
pub fn generate_license(options: Args) -> Result<()> {
	let tangram_license_private_key = std::fs::read_to_string(options.private_key)?;
	...
	std::fs::write(options.output, license)?;
	Ok(())
}
